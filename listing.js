...
function projectCard(project){
  ...
  return '<div class="pc reveal" onclick="location.href=\'Project Detail.html?id='+Number(project.id)+'\'">'
    ...
    +'<div class="pc-tags">'+(project.tags||[]).slice(0,5).map(function(tag){return'<span class="ptag">'+escapeHtml(tag)+'</span>';}).join('')+'</div>'
...
function blogCard(post){
  ...
  return '<div class="bc reveal" onclick="location.href=\'Blog Detail.html?id='+Number(post.id)+'\'">'
    ...
    +'<div class="bc-body"><div class="bc-tags">'+(post.tags||[]).slice(0,3).map(function(tag){return'<span class="btag">'+escapeHtml(tag)+'</span>';}).join('')+'</div>'
...
async function loadListing(kind){
  ...
  var result=kind==='projects'
    ...
    : await db.from('blog_posts').select('*').eq('status','published').eq('type','blog').order('created_at',{ascending:false});
  if(result.error){grid.innerHTML='<p class="ssub">Unable to load content.</p>';return;}
